...
#+END_SRC
*** Hacky fix for RET and comma in heredoc/nowdoc
...
#+BEGIN_SRC emacs-lisp
  (use-package php-ts-mode
    ...
    (defun r/fix-janky-php-input ()
      (let ((ret-fallback (or (key-binding (kbd "RET") t) #'newline))
            (comma-fallback (or (key-binding (kbd ",") t) #'self-insert-command)))
        (local-set-key
         (kbd "RET")
         (lambda ()
           (interactive)
           (if (r/php-ts--in-heredoc-body-p)
               (let ((indent (current-indentation)))
                 (newline)
                 (indent-to indent))
             (call-interactively ret-fallback))))
        ...
        (local-set-key
         ...
         (lambda ()
           ...
           (if (r/php-ts--in-heredoc-body-p)
             ...
             (call-interactively comma-fallback)))))))
#+END_SRC
** Documentation
#+BEGIN_SRC emacs-lisp

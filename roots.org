...
#+begin_src emacs-lisp
  (use-package agent-shell
    :unless (getenv "NO_COPILOT")
    :hook (agent-shell-mode
           . (lambda () (kill-ring-deindent-mode -1)))
    :bind (:map agent-shell-mode-map
                ("C-c '" . agent-shell-prompt-compose))
    :custom ((agent-shell-preferred-agent-config
              (agent-shell-goose-make-agent-config))
             (agent-shell-goose-authentication
              (agent-shell-make-goose-authentication :none t))
             (agent-shell-header-style 'text))
    :config (defun r/agent-shell-goose-in-directory (dir)

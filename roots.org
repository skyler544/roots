...
** LLM Assistants
*** gptel
This config is starting to get a little long, but the gist is:
1. I want my chats to be autosaved with a filename that reflects the topic of the chat
2. I want to be able to quickly define new presets for various tasks

My prompts live in [[https://github.com/skyler544/prompts][this repo]]. See the PHP section for an example of how to
register mode-specific prompts.
...
#+BEGIN_SRC emacs-lisp
  (use-package gptel
    ...
    (defun r/gptel-register-presets (&optional presets)
      ...
      (mapc (lambda (preset) (r/gptel--make-preset preset gptel-backend))
            r/gptel-presets))
...
#+END_SRC
**** Prompts
These prompts are contained in a [[https://github.com/skyler544/prompts][separate repository]]. The idea is to have a
hierarchy of similar prompts tweaked to specific tasks.
#+BEGIN_SRC emacs-lisp
  (unless (getenv "NO_COPILOT")
    (with-eval-after-load 'gptel
      ;; DEFAULTS
      (r/gptel-register-presets
       '((misc/one-line-summary)
         (misc/terse-summary)
         (misc/thorough-summary)
         (misc/brevity)
         (misc/step-by-step)
         (research/scientific-advisor)
         (research/challenge research/scientific-advisor)
         (research/review research/scientific-advisor)
         (translation/en-de)
         (programming/simple)))
      ;; PHP
      (r/gptel-register-presets
      ...
      (r/gptel-register-presets
       '((shell/standards)
         (shell/programming shell/standards)))
      ;; EMACS LISP
      (r/gptel-register-presets
       '((elisp/standards nil)
         (elisp/programming elisp/standards)
         (elisp/refactor elisp/programming)
         (elisp/review elisp/standards)))))
#+END_SRC
** Distrobox

...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package php-ts-mode
    ...
    (defun r/laravel-test-command (type args)
      (let ((default-directory (r/project-root)))
        (async-shell-command
         (format "./sail %s test %s %s" type args
                 (shell-quote-argument (r/project-relative-path))))))
    (defun r/laravel-test-at-point ()
      (interactive)
      (let* ((default-directory (r/project-root))
             (class (file-name-base (buffer-name)))
             (method (r/php-function-name-at-point))
             (filter (concat class "::" method))
             (cmd (format "./sail artisan test --filter %s" (shell-quote-argument filter))))
    ...
    (defun r/laravel-debug-test-current-file ()
      (interactive)
      (let* ((default-directory (r/project-root))
             (cmd (format "./sail debug test --ansi %s"
                          (shell-quote-argument (r/project-relative-path)))))))

...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Generate an autoload for `term-mode'
  (use-package term
    :commands term-mode)

  ;; Configurable command recipes
  (use-package run-command
    :hook (term-mode . r/run-command-compilation-minor-mode)
    :bind ("C-c r" . run-command)
    :config
    (defun r/run-command-term-buffer-p (buffer &optional arg)
      "Returns t if BUFFER is a term-mode buffer and was generated by
  `run-command'."
      (with-current-buffer buffer
        (and (derived-mode-p 'term-mode) (run-command-buffer-p buffer))))
  ...
  generated by `run-command'."
      (when (r/run-command-term-buffer-p (current-buffer))
        (compilation-minor-mode)))

    (r/display-buffer-same-window #'r/run-command-term-buffer-p))

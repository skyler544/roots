...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package php-ts-mode
    ...
    (with-eval-after-load 'gptel
      (setq r/gptel-presets
            (append r/gptel-presets
                    '((php/standards nil)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package gptel
    ...
    ;; UTILITY FUNCTIONS
    (defun r/load-prompt (file-base-name)
      (let* ((dir (expand-file-name "~/build/programming/prompts"))
             (file (expand-file-name (concat (symbol-name file-base-name) ".md") dir)))
    ...
    (defun r/gptel-make-preset (spec backend)
      (let* ((name (car spec))
             (parent (cadr spec))
             (system (r/load-prompt name))
             (args (list name :backend backend :system system)))
        (when parent
          (setq args (append args (list :parents parent))))
        (apply #'gptel-make-preset args)))

    (defun r/gptel-register-presets ()
      (interactive)
      (when r/gptel-presets
        (dolist (spec r/gptel-presets)
          (r/gptel-make-preset spec "Copilot"))))

    (defun r/gptel--save-one-line-summary (response info)
      (let ((chat-buf (plist-get info :buffer))
            (cache-dir (expand-file-name "var/cache/gptel" user-emacs-directory)))
        (unless (file-directory-p cache-dir)
          (make-directory cache-dir t))
        (with-current-buffer chat-buf
          (let* ((title (if (stringp response) (string-trim response) ""))
                 (safe (replace-regexp-in-string "[^A-Za-z0-9._-]" "-" title))
                 (safe (if (> (length safe) 80) (substring safe 0 80) safe))
                 (basename (concat "Copilot-" safe ".org"))
                 (path (expand-file-name basename cache-dir)))
    ...
    (defun r/gptel-load-chat ()
      (interactive)
      (let* ((default-directory (expand-file-name "var/cache/gptel/" user-emacs-directory)))

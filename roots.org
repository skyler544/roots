...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (defun r/org-mode-face-override ()
      "Makes all the headings bold and fixes themes messing with the
   face height."
      (interactive)
      (let ((facelist (r/face-filter "^org-level-")))
        (dolist (face facelist)
...
The size used here may be inappropriate for your display; tweak if needed. Good
values seem to be 100 - 160 on mine.

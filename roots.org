...
#+BEGIN_SRC emacs-lisp
  (use-package ghostel :after devil
    ...
    (defun r/ghostel-shell-command-buffer-name (command)
    ...
    (defun r/ghostel-shell-command (command &optional directory)
      ...
      (let ((default-directory (or directory default-directory)))
        (ghostel-compile--start
         command
         (r/ghostel-shell-command-buffer-name command)
         default-directory)))

    (defun r/project-ghostel-shell-command (command)
      (interactive
       (let* ((project (project-current t))
              (default-directory (project-root project)))
         (list (read-shell-command "Ghostel project shell command: "
                                   nil 'shell-command-history))))
      (let ((project (project-current t)))
        (r/ghostel-shell-command command (project-root project))))

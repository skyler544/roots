...
#+BEGIN_SRC emacs-lisp
  ...
  (setq r/buffer-regexes '("\\*Help" "\\*helpful" "^magit:" "^*shell:" "^*SQL:"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package run-command
    :hook (eat-mode . r/run-command-compilation-minor-mode)
    :bind ("C-c r" . run-command)
    :config
    (setq run-command-default-runner 'run-command-runner-eat)
    (defun r/run-command-eat-buffer-p (buffer &optional arg)
      "Returns t if BUFFER is a term-mode buffer and was generated by
  `run-command'."
      (with-current-buffer buffer
        (and (eq major-mode 'eat-mode) (run-command-buffer-p buffer))))

    (defun r/run-command-compilation-minor-mode ()
      "Runs `compilation-minor-mode' in the current buffer if it was
  generated by `run-command'."
      (when (r/run-command-eat-buffer-p (current-buffer))
        (compilation-minor-mode)))

    (r/display-buffer-same-window #'r/run-command-eat-buffer-p))
